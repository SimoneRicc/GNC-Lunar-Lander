%% Monte-Carlo around the nominal case (thrust & Isp ±3σ, 95% CI overlays on scatter)
clear; clc; rng('default');

%% ===== General parameters =====
g_moon = 1.62;          % m/s^2
dt      = 0.5;          % s
v_td    = -0.1;         % target m/s in phase 3 (touchdown vertical speed)
...
%% ===== Requested FIXES (targets kept fixed as specified) =====
z0_fix  = 15e3;               % FIXED
vx0_fix = -1695;              % FIXED
...
%% ===== Nominal case (trajectory targets) =====
nom = struct();
...
%% ===== Dispersions (±3σ, truncated normal around 1.0) =====
THR_3SIG_PCT = 0.09;   % e.g. ±9% at 3σ on Tmax
ISP_3SIG_PCT = 0.06;   % e.g. ±6% at 3σ on Isp
SIG_T   = THR_3SIG_PCT/3;
SIG_ISP = ISP_3SIG_PCT/3;
draw_scale_pm3sig = @(sigma) min(1+3*sigma, max(1-3*sigma, 1 + sigma*randn()));

%% ===== Nominal simulation (no dispersion) =====
[sim_nom, out_nom] = simulate_with_thrust_limits(nom, params, dt, v_td, 1.0, 1.0);
...
% Variables to vary around nominal (others fixed as requested)
PCT = struct('x0',0.2,'x1',0.0,'z1',1,'vx1',0.5,'vz1',0.3);
ABS = struct('y0',10e3,'y1',0e3,'vy1',0);
...
for i=2:nMC+1
    ...
    [sim, out] = simulate_with_thrust_limits(s, params, dt, v_td, Tscale_i, IspScale_i);
...
for ph=1:3
    idx=(bs.phase==ph);
    plot3(bs.x(idx)/1000,bs.y(idx)/1000,bs.z(idx)/1000,'.','Color',C(ph,:));
end
...
if exist('yline','file'), yline(95,'--r'); else, plot([bs.t(1) bs.t(end)],[95 95],'r--'); end
...
if exist('yline','file'), yline(0,'--k'); else, plot([bs.t(1) bs.t(end)],[0 0],'k--'); end
legend('vx','vy','vz'); title('Velocities');

%% ===== Confidence intervals on ΔV (success only) =====
okIdx = find(res.success);
if ~isempty(okIdx)
    dv_ok = res.dv(okIdx);
    q68   = prctile(dv_ok,[16 50 84]);      % ~1σ
    q95   = prctile(dv_ok,[2.5 50 97.5]);   % ~95%
    q997  = prctile(dv_ok,[0.15 99.85]);    % ~3σ
    ...
    % ===== Scatter ΔV vs dispersions — 95% CI band + normal bell =====
    figure('Name','ΔV vs dispersions — 95% CI & normal bell');
    tiledlayout(1,2,'Padding','compact','TileSpacing','compact');

    % Values for annotations
    dv_best     = res.dv(best);
    dv95_lo     = q95(1);         % 2.5th percentile
    dv95_med    = q95(2);         % median
    dv95_hi     = q95(3);         % 97.5th percentile
    delta95_abs = max(0, dv95_hi - dv_best);
    delta95_pct = 100 * delta95_abs / max(dv_best, eps);

    % --------- Panel 1 : ΔV vs Tscale ----------
    nexttile; hold on; grid on;
    xT = res.Tscale(okIdx); yT = dv_ok;
    scatter(xT, yT, 8, 'filled');
    xlabel('T_{scale} (thrust dispersion)'); ylabel('\DeltaV (m/s)'); title('\DeltaV vs T_{scale}');

    % Red horizontal band = global 95% CI on ΔV
    yl = ylim; xl = xlim;
    patch([xl(1) xl(2) xl(2) xl(1)], [dv95_lo dv95_lo dv95_hi dv95_hi], [1 0 0], ...
          'FaceAlpha',0.08, 'EdgeColor','none');

    % Reference lines
    if exist('yline','file')
        yline(dv_best,'--k','Best \DeltaV','LabelHorizontalAlignment','left','LabelVerticalAlignment','bottom');
        yline(dv95_med,':k','Median','LabelHorizontalAlignment','left','LabelVerticalAlignment','top');
    else
        plot([xl(1) xl(2)],[dv_best dv_best],'k--');
        plot([xl(1) xl(2)],[dv95_med dv95_med],'k:');
    end


    % On-plot English summary
    txtEN = sprintf(['95%% CI on \\DeltaV: [%.1f, %.1f] m/s  \nBest \\DeltaV: %.1f m/s' ...
                     '\nAdd +%.1f m/s (%.1f%%) to cover 95%%'], ...
                     dv95_lo, dv95_hi, dv_best, delta95_abs, delta95_pct);
    text(xl(1)+0.02*(xl(2)-xl(1)), yl(2)-0.08*(yl(2)-yl(1)), txtEN, ...
         'FontWeight','bold','BackgroundColor','w');

    % --------- Panel 2 : ΔV vs IspScale ----------
    nexttile; hold on; grid on;
    xI = res.IspScale(okIdx); yI = dv_ok;
    scatter(xI, yI, 8, 'filled');
    ...
    % Same summary text (global 95% CI)
    text(xl(1)+0.02*(xl(2)-xl(1)), yl(2)-0.08*(yl(2)-yl(1)), txtEN, ...
         'FontWeight','bold','BackgroundColor','w');

    % ---- Console print of the operational value searched (uplift to cover 95%) ----
    fprintf(['\n[95%% Summary]\nBased on the best solution, adding +%.1f m/s (%.1f%%) ' ...
             'to the nominal %.1f m/s brings the required ΔV to %.1f m/s (95%% coverage).\n'], ...
...
for idx = okPlot(:)'
    si = sims{idx};
    td_ok(end+1,:) = [si.x(end)/1000, si.y(end)/1000, si.z(end)/1000]; %#ok<SAGROW>
end
for idx = koPlot(:)'
    si = sims{idx};
    td_ko(end+1,:) = [si.x(end)/1000, si.y(end)/1000, si.z(end)/1000]; %#ok<SAGROW>
end
...
fprintf('  Dispersions: THR_3SIG=±%.1f%%, ISP_3SIG=±%.1f%%\n', 100*THR_3SIG_PCT, 100*ISP_3SIG_PCT);
...
function [sim, out] = simulate_with_thrust_limits(S, params, dt, v_td, Tscale, IspScale)
% Integrates the descent obeying:
%   - Tmax_eff = Tscale * params.Tmax
%   - Isp_eff  = IspScale * params.Isp
% Fuel: mdot = T / (Isp_eff * g0)
g_moon = params.g_moon; Isp0 = params.Isp; g0 = params.g0;
Tmax0  = params.Tmax;   m_dry = params.m_dry; m = params.m0;
Tmax_eff = Tmax0 * Tscale;
Isp_eff  = max(1e-6, Isp0 * IspScale);

% --- Coefficients (polynomial guidance laws) ---
...
ax_all=0; ay_all=0; az_all=0; phase_all=0; m_all=m; thr_all=0;
amag_all=0; tilt_all=0;

% Apply thrust limits & consume propellant
    function [ax_t,ay_t,az_t,throttle,amag,T_mag,m_next,tiltdeg] = apply_thrust_limits(ax_cmd,ay_cmd,az_cmd,m_now)
        if m_now <= m_dry + 1e-9
            ax_t=0; ay_t=0; az_t=0; amag=0; T_mag=0; throttle=0; tiltdeg=0; m_next=m_now;
            return;
        end
        amag_cmd = sqrt(ax_cmd^2 + ay_cmd^2 + az_cmd^2);
        if amag_cmd < 1e-12
            ax_t=0; ay_t=0; az_t=0; amag=0;  T_mag=0; throttle=0; tiltdeg=0; m_next=m_now;
            return;
        end
        amax = Tmax_eff / m_now;
        s = min(1.0, amax / amag_cmd);
        ax_t = s*ax_cmd; ay_t = s*ay_cmd; az_t = s*az_cmd;
        amag = sqrt(ax_t^2 + ay_t^2 + az_t^2);
        T_mag = m_now * amag;
        throttle = T_mag / max(Tmax_eff, 1e-9);
        % Fuel consumption with Isp_eff
        mdot = T_mag / (Isp_eff * g0);
        m_next = max(m_dry, m_now - mdot*dt);
        % Tilt (0 = all vertical)
        tiltdeg = atan2d(hypot(ax_t, ay_t), max(az_t, 0));
    end

% ===== PHASE 1: braking =====
Tgo1 = max(10, (S.x1_t - S.x0) / (0.5*(S.vx0 + S.vx1_t))); tgo=Tgo1;
while (z > S.z1_t)
    ...
    [ax_t,ay_t,az_t,thr,amag,~,m_new,tiltdeg] = apply_thrust_limits(ax_cmd,ay_cmd,az_cmd,m);

    vx=vx+ax_t*dt; vy=vy+ay_t*dt; vz=vz+(az_t - g_moon)*dt;
    x=x+vx*dt; y=y+vy*dt; z=z+vz*dt; t=t+dt; tgo=tgo-dt; m=m_new;

    t_all(end+1,1)=t; x_all(end+1,1)=x; y_all(end+1,1)=y; z_all(end+1,1)=z;
    vx_all(end+1,1)=vx; vy_all(end+1,1)=vy; vz_all(end+1,1)=vz;
    ax_all(end+1,1)=ax_t; ay_all(end+1,1)=ay_t; az_all(end+1,1)=az_t; phase_all(end+1,1)=1;
    m_all(end+1,1)=m; thr_all(end+1,1)=thr; amag_all(end+1,1)=amag; tilt_all(end+1,1)=tiltdeg;
    if z<=0, break; end
end
if z<=0, finalize(); return; end

% ===== PHASE 2: approach =====
Tgo2 = max(10, (S.x2_t - S.x1_t) / (0.5*(S.vx1_t + S.vx2_t))); tgo=Tgo2;
while (z > S.z2_t)
    ...
    t_all(end+1,1)=t; x_all(end+1,1)=x; y_all(end+1,1)=y; z_all(end+1,1)=z;
    vx_all(end+1,1)=vx; vy_all(end+1,1)=vy; vz_all(end+1,1)=vz;
    ax_all(end+1,1)=ax_t; ay_all(end+1,1)=ay_t; az_all(end+1,1)=az_t; phase_all(end+1,1)=2;
    m_all(end+1,1)=m; thr_all(end+1,1)=thr; amag_all(end+1,1)=amag; tilt_all(end+1,1)=tiltdeg;
    if z<=0, break; end
end
if z<=0, finalize(); return; end
...
while z>0
    ...
    vx=vx+ax_t*dt; vy=vy+ay_t*dt; vz=vz+(az_t - g_moon)*dt;
    x=x+vx*dt; y=y+vy*dt; z=z+vz*dt; t=t+dt; m=m_new;

    t_all(end+1,1)=t; x_all(end+1,1)=x; y_all(end+1,1)=y; z_all(end+1,1)=z;
    vx_all(end+1,1)=vx; vy_all(end+1,1)=vy; vz_all(end+1,1)=vz;
    ax_all(end+1,1)=ax_t; ay_all(end+1,1)=ay_t; az_all(end+1,1)=az_t; phase_all(end+1,1)=3;
    m_all(end+1,1)=m; thr_all(end+1,1)=thr; amag_all(end+1,1)=amag; tilt_all(end+1,1)=tiltdeg;
end
...
finalize();
    ...
    function finalize()
        ...
        % ΔV = ∫|a_thrust| dt (after saturation)
        sim.dv_total = trapz(sim.t, amag_all);

        out.T_vec    = [ax_all.*m_all, ay_all.*m_all, az_all.*m_all];
        out.T_mag    = sqrt(sum(out.T_vec.^2,2));
        out.throttle = thr_all;
        out.m        = m_all;
        out.dV       = sim.dv_total;
        out.m_prop   = params.m0 - m_all(end);
        out.a_cmd    = [sim.ax, sim.ay, sim.az];
        out.tiltdeg  = tilt_all;
        out.flags.sat_throttle = out.throttle>1;     % should be 0 since capped
        out.flags.vert_negative= out.T_vec(:,3)<0;
        out.flags.over_tilt    = tilt_all>params.tilt_max_deg;
    end
end

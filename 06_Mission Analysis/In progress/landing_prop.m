...
x0  = 2.886e5;        % initial downrange to target (m)
z0  = 15e3; 
y0  = 10e3; 
% initial altitude (m)
...
x1_t  = 1.833e4;   y1_t  = 0;     z1_t  = 5.607e3   % 
vx1_t = -169.931;    vy1_t =  0;    vz1_t = -58.301;      % m/s
...
while (z > z1_t)
    ...
    ax_all(k,1)= ax_cmd; ay_all(k,1) = az_cmd; az_all(k,1)= az_cmd;
    phase_all(k,1)=1;
...
% Trajectory data (example):
params.g_moon = 1.62;
params.Isp    = 450;
params.g0     = 9.80665;
params.Tmax   = 100e3;
params.m0     = 18000;      % e.g., 34 t wet
...
% Quick plots
figure('Name','Thrust orientation');
subplot(3,1,1); grid on; plot(t_all, ang_vert, 'LineWidth',1.2);
ylabel('Angle to vertical (deg)'); title('Thrust vs vertical/surface/azimuth');
